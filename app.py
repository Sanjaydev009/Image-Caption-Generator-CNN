...
def process_image(image_path):
    ...
    try:
        ...
        # Generate captions - limit to 4-5 recommended captions only
        captions = []
        caption_sources = []
        max_captions = 5  # Limit to 5 captions maximum
        
        # 1. Try the ensemble captioner first (best quality) - generate 2 captions
        if ensemble_captioner is not None and len(captions) < max_captions:
            try:
                print("Generating captions using ensemble captioner model...")
                # Generate only 2 high-quality captions with the ensemble captioner
                ensemble_captions = ensemble_captioner.generate_captions(
                    img,
                    scene_type=scene_type,
                    objects=detected_objects,
                    colors=dominant_colors,
                    num_captions=2  # Reduced from 10 to 2
                )
                
                for caption in ensemble_captions:
                    if caption and caption not in captions and len(captions) < max_captions:
        ...
        # 2. Try transformer model - generate 1 additional caption
        if len(captions) < max_captions and transformers_captioner and transformers_captioner.initialized:
            try:
                print("Generating caption using transformer model...")
                # Generate only 1 caption with the transformer model
                transformer_captions = transformers_captioner.generate_multiple_captions(img, 1)
                for caption in transformer_captions:
                    if caption and caption not in captions and len(captions) < max_captions:
                        captions.append(caption)
                        caption_sources.append("transformer")
                print(f"Added transformer caption. Total: {len(captions)}")
            except Exception as e:
                print(f"Error generating transformer captions: {e}")
        
        # 3. Try scene understanding captioner - generate 1 caption
        if len(captions) < max_captions and scene_captioner is not None:
            try:
                # Use scene captioning for one additional perspective
                ...
                if scene_caption and scene_caption not in captions and len(captions) < max_captions:
                    captions.append(scene_caption)
                    caption_sources.append("scene")
                    print(f"Added scene caption. Total: {len(captions)}")
            except Exception as e:
                print(f"Error with scene captioning: {e}")
        
        # 4. Try enhanced captioner - generate 1 final caption if needed
        if len(captions) < max_captions and enhanced_captioner is not None:
            try:
                # Use enhanced captioning for final perspective
                ...
                if enhanced_caption and enhanced_caption not in captions and len(captions) < max_captions:
                    captions.append(enhanced_caption)
                    caption_sources.append("enhanced")
                    print(f"Added enhanced caption. Total: {len(captions)}")
            except Exception as e:
